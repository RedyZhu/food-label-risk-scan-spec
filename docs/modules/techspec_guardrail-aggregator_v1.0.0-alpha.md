...
- GuardrailAggregator 是 `fingerprint` 的规范生成点（source of generation）。
- 若上游已提供 `fingerprint`，先校验格式；若缺失则在本节点按固定规则生成。
- 推荐生成输入：`risk_type + detection_method + block_id + normalized_snippet`。
- 同指纹只保留一条。
...
4) **正常映射场景**
- 匹配优先级：(`source_module` + `source_risk_index`) > `fingerprint`（若可用）> (`risk_type` + `detection_method` + `evidence.block_id` + normalized(`evidence.raw_snippet`)) > `risk_type`。
- 若仅命中 `risk_type`（弱匹配），必须记录告警：`SEVERITY_WEAK_MATCH`。
- 若同一匹配键出现多个 `severity`，按固定优先级处理并记录冲突：
